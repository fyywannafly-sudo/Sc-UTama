...
local AFS_FishCheckDelay = 3.5 -- Default: 2.5 + 1
...
local function AFS_FishCheckLoop()
    ...
    while AFS_Running and AFS_FishCheckEnabled do
        local currentTime = tick()
        if currentTime - AFS_LastFishTime >= AFS_FishCheckDelay and AFS_LastFishTime > 0 then
            retryCount = retryCount + 1
...
local AFS_DelaySlider = Auto:Slider({
    ...
    Callback = function(value)
        ...
        AFS_FishCheckDelay = value + 1 -- Menambahkan 1 detik untuk fish check delay
    end
})
...
local AnimToggle = Setting:Toggle({
...
local NotifToggle = Setting:Toggle({
    Title = "Remove Small Notification",
    Default = false,
    Callback = function(on)
        task.wait(1)
